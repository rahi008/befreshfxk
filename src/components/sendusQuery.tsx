        ...
        <Transition show={isOpen} as={React.Fragment}>
            <div className="fixed inset-0 z-50 overflow-y-auto">
                <div className="flex items-center justify-center min-h-screen ">
                    <Transition.Child as={React.Fragment} enter="transition-opacity duration-300" enterFrom="opacity-0"
                        enterTo="opacity-100" leave="transition-opacity duration-300" leaveFrom="opacity-100"
                        leaveTo="opacity-0">
                        <div className="fixed inset-0 bg-black opacity-30" onClick={onClose}></div>
                    </Transition.Child>

                    <Transition.Child as={React.Fragment} enter="transition-transform duration-300"
                        enterFrom="translate-y-full" enterTo="translate-y-0" leave="transition-transform duration-300"
                        ...
                        <div className="z-10 w-full max-w-xl mx-auto bg-white rounded-md shadow-xl">
                            <div className="flex justify-between bg-teal-600  pl-6 ">
                                <h3 className="text-lg text-white font-semibold">Send Us Query</h3>
                                <button className="text-white bg-red-600 px-4 py-2 hover:text-red-700"
                                    onClick={onClose}>
                                    x
                                </button>
                            </div>
                            <div className='p-2 md:p-6'>
                                ...
                                <form onSubmit={handleFormSubmit}>
                                    <div className='border border-slate-600 p-2 mt-2 rounded'>
                                        <div className="mt-4 flex flex-row justify-between">
                                            ...
                                            <span className="space-x-4">
                                                ...
                                                <button key={index} className={`py-1 px-3 border ${
                                                    selectedOption===option ? 'bg-blue-500 text-white'
                                                    : 'border-gray-300' } rounded-md`} onClick={()=>
                                        ...
                                        <div className="mt-4 flex flex-col md:flex-row md:justify-between">
                                            <CmbBox currencyList={currencyList} onChange={handleCurrencyChange} />
                                    ...
                                    <div className="mt-6 text-right">
                                        <button type="submit" className="bg-green-500 text-white px-4 py-2 rounded-sm">
                                            Submit
                                        </button>
                                    </div>
                                </form>
                                <label className='text-sm'>BeFreshFX concern person will contact to you soon.</label>
